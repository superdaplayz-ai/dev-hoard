...
import type { Snippet, SnippetVersion } from "@/data/db";
import { useToast } from "@/hooks/use-toast";

const Shared = () => {
  const { toast } = useToast();
  const { load, snippets, filters, setFilters, addSnippet, updateSnippet, deleteSnippet } = useSnippetsStore();
  ...
  const filtered = useMemo(() => {
    ...
    return s.filter((item) => {
      // Only shared ones (by tag convention)
      if (!item.tags.includes("shared")) return false;
  ...
  const onSave = async (val: EditorValue) => {
    if (editing) {
      ...
      await addSnippet({
        title: val.title,
        description: val.description,
        tags: Array.from(new Set([...(val.tags || []), "shared"])),
        project: val.project,
        favorite: false,
        codeBlocks: val.codeBlocks,
      ...
      toast({ title: "Lagt til", description: "Ny delt snippet opprettet." });
    }
    setEditorOpen(false);
    setEditing(null);
  };
  ...
  return (
    <SidebarProvider>
      <Helmet>
        <title>Delte snippets – Snippet Manager</title>
        <meta name="description" content="Se alle delte kodeeksempler merket med taggen shared." />
      ...
      <div className="min-h-screen flex w-full">
        ...
        <div className="flex-1 flex flex-col">
          ...
          <main className="container mx-auto px-4 py-6 space-y-4">
            <header>
              <h1 className="text-2xl font-semibold">Delte kodeeksempler</h1>
              <p className="text-muted-foreground">Viser snippets merket med taggen <span className="font-mono">shared</span>.</p>
            </header>
            ...
            {filtered.length === 0 ? (
              <div className="rounded-md border p-8 text-center text-muted-foreground">
                <p>Ingen delte snippets enda. Merk en snippet med taggen <span className="font-mono">shared</span>.</p>
              </div>
            ) : (
